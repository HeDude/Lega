...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('maverick-best').innerHTML = html;
    ...
    Object.values(categories).slice(0, 10).forEach(terms => {
        ...
        terms.forEach(term => {
            ...
            // Create the cell with a linear gradient background
            const cellClass = isFound ? 'cell row-found' : 'cell';
            const cellStyle = `background: linear-gradient(to top, red ${fillPercentage}%, transparent ${fillPercentage}%);`;
            html += `<div class="${cellClass}" data-points="${cellPoints}" style="${cellStyle}">${term}</div>`;
        });
    });

    document.getElementById('maverick-worst').innerHTML = html;
});
